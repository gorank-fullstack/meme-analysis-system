...
export interface CustomFilterProps {
  ...
  buttonClassName?: string; // 默认触发按钮样式覆盖
  trigger?: React.ReactNode; // 外部自定义触发器（可选）
  hideDefaultTrigger?: boolean; // 强制不渲染内置 Trigger（用于完全外部控制）
}
...
export default function CustomFilter(props: CustomFilterProps) {
  const { value, onChange, onApply, onResetToDefault, buttonClassName, trigger, hideDefaultTrigger } = props;
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      {/* Trigger: 保证始终渲染在 Dialog.Content 之外 */}
      {!hideDefaultTrigger && (
...
// Usage Examples
// ===============================
export function CustomFilter_DefaultDemo() {
  ...
  return (
    <div className="p-4">
      <CustomFilter
        value={filter}
        onChange={setFilter}
        onApply={(v) => console.log("apply filter:", v)}
...
// 外部自定义触发器（推荐：用于放在工具条中）
export function CustomFilter_WithExternalTrigger() {
  const [filter, setFilter] = React.useState<ICustomFilter>(DEFAULT_FILTER);
  return (
...
// 完全受控（外部按钮控制 open）
export function CustomFilter_Controlled() {
  const [filter, setFilter] = React.useState<ICustomFilter>(DEFAULT_FILTER);
  const [open, setOpen] = React.useState(false);
  return (
    <div className="p-4 flex items-center gap-3">
      <button className="btn btn-sm" onClick={() => setOpen(true)}>打开筛选</button>
      <CustomFilter
        open={open}
