...
exports.mathRound_Min001_d2_n2n = exports.parseNumberOrZero = void 0;
exports.mathCut_ParseInt_s2n = mathCut_ParseInt_s2n;
exports.mathRound_ParseFloat_d2_s2n = mathRound_ParseFloat_d2_s2n;
exports.mathRound_d2_n2n = mathRound_d2_n2n;
exports.mathCut_Percent_d2_s2n = mathCut_Percent_d2_s2n;
exports.mathRound_Percent_d2_s2n = mathRound_Percent_d2_s2n;
exports.mathRound_CmcMillion_d2_n2n = mathRound_CmcMillion_d2_n2n;
/**
 * 命名规则说明：
...
// 百分比上限值可提取常量
const MAX_PERCENT_RESULT = 1000000;
// 用判断函数：isValidNumericStr
function isValidNumericStr(input) {
    return NUMERIC_REGULAR_EXPRESSION.test(input.trim());
}
//返回字符串的：取整（非四舍五入）
function mathCut_ParseInt_s2n(input) {
...
function mathRound_ParseFloat_d2_s2n(input) {
    if (typeof input !== 'string')
        return 0;
    const trimmed = input.trim();
    if (!trimmed)
...
function mathRound_d2_n2n(input) {
...
function mathCut_Percent_d2_s2n(input) {
    if (typeof input !== 'string')
        return 0;
    const trimmed = input.trim();
    if (!trimmed)
...
function mathRound_Percent_d2_s2n(input) {
    if (typeof input !== 'string')
        return 0;
    const trimmed = input.trim();
    if (!trimmed)
...
const parseNumberOrZero = (s) => {
    const x = Number.parseFloat(s);
    return Number.isFinite(x) ? x : 0;
};
exports.parseNumberOrZero = parseNumberOrZero;
/**
 * 市值换算为百万单位，保留 2 位小数，小于 0.01 则取 0.01
 */
function mathRound_CmcMillion_d2_n2n(cmc) {
    // 非数字、Infinity、NaN 都直接回退到 0.01
    if (!Number.isFinite(cmc))
        return 0.01;
    const millionValue = cmc / 1000000;
    return Math.max(0.01, mathRound_d2_n2n(millionValue));
}
// 改成用更稳健的函数
const mathRound_Min001_d2_n2n = (x) => Math.max(0.01, mathRound_d2_n2n(x));
exports.mathRound_Min001_d2_n2n = mathRound_Min001_d2_n2n;
