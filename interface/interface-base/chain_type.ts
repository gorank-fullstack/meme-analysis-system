...
// ─── Chain Type ─────────────────────────────────────────────
export type TChainName =
    // SPL
    | "sol"

    // EVM
    | "eth" | "bsc" | "base" | "aval" | "arb" | "linea" | "pulse"
    | "polygon" | "optimism" | "moonbeam" | "gnosis" | "ronin"
    | "fantom" | "cronos" | "lisk" | "chiliz" | "blast" | "sonic"
    | "hyper" | "sei" | "uni" | "x-layer"
...
// 支持的区块链类型列表
export const SUPPORT_CHAIN: TChainName[] = ['sol', 'eth', 'bsc'];
/**
 ...
 * includes() search.
 */
export function isTChainName(val: string): val is TChainName {
...
// 判断是否 SPL 链
export function isTChainSpl(val: string): val is TChainSpl {
    return val === "sol";
}

// 判断是否 EVM 链
export function isTChainEvm(val: string): val is TChainEvm {
    return (
        val === "eth" ||
        val === "bsc" ||
        val === "base" ||
...
// 判断是否 TVM 链
export function isTChainTvm(val: string): val is TChainTvm {
    return val === "tron";  //mo平台20250911不支持：tron
}

// 判断是否 FVM 链
export function isTChainFvm(val: string): val is TChainFvm {
    return val === "flow";  //mo平台20250911不支持：flow--token prices
}

// 判断是否 TAO 链
export function isTChainTao(val: string): val is TChainTao {
    return val === "bittensor";  //mo平台20250911不支持：bittensor
}

// 判断是否 MOVE 链
export function isTChainMove(val: string): val is TChainMove {
...
export const SUPPORT_QT: TQtType[] = ['5m', '1h', '6h', '24h']; //所有支持的时间段

export const SUPPORT_QT_FILE_LIST: TQtType[] = ['6h', '24h'];   //可以:保存/读取的--文件:时间段
export const SUPPORT_QT_CACHE_LIST: TQtType[] = ['6h', '24h'];   //可以:保存/读取的--缓存:时间段

export function isTQtType(val: string): val is TQtType {
...
export function isTTabTypeServer(tab: string): tab is TTabType_Server {
    // 性能比：return ["trending_pools", "new_pools"].includes(tab);更高
    return tab === "trending_pools" || tab === "new_pools";
}

export function isTTabTypeClient(tab: string): tab is TTabType_Client {
    return tab === "hot" || tab === "new";
}

// ─── Composite Type ──────────────────────────────────────────
/* export type TChainQtKey =
...
export const ALL_CHAIN_QT_KEYS: readonly TChainQtKey[] = [
    // SPL
    "sol_5m", "sol_1h", "sol_6h", "sol_24h",
